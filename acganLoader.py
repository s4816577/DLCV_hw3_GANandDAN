...
def parse_label(filename):
    ...
    with open(filename, 'r', encoding='utf8') as f:
        for line in f:
            ...
            if len(token) != 14 or first_row:
                first_row = False
                continue
            result.append(np.float32(token[10]))
    return result  

def test_loader():
    trainset = FACE('hw3_data/face/train', 'hw3_data/face/train.csv', transform=transforms.Compose([transforms.ToTensor(), transforms.Normalize([0.5], [0.5])]))
