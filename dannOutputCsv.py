...
def main():
    ...
    if sys.argv[2] == 'mnistm':
        load_checkpoint('Models/DANN1-F.pth', model_F, 111)
        load_checkpoint('Models/DANN1-L.pth', model_L, 111)
        load_checkpoint('Models/DANN1-D.pth', model_D, 111)
        need_exten_channel = False
    elif sys.argv[2] == 'svhn':
        load_checkpoint('Models/DANN2-F.pth', model_F, 111)
        load_checkpoint('Models/DANN2-L.pth', model_L, 111)
        load_checkpoint('Models/DANN2-D.pth', model_D, 111)
        need_exten_channel = False
    elif sys.argv[2] == 'usps':
        load_checkpoint('Models/DANN3-F.pth', model_F, 111)
        load_checkpoint('Models/DANN3-L.pth', model_L, 111)
        load_checkpoint('Models/DANN3-D.pth', model_D, 111)
        need_exten_channel = True
    
    #usps -> mnistm -> svhn
    #dataset init and loader warper
    target_set = dannLoaderTest.DIGIT(sys.argv[1], transforms.ToTensor())
    target_loader = DataLoader(target_set, batch_size=bt_size, shuffle=False, num_workers=1)
    
    #Debug(model, testset_loader)
    test(model_F, model_L, target_loader, device, need_exten_channel)
